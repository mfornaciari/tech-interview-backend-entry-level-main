...
RSpec.describe MarkCartAsAbandonedJob, type: :job do
  ...
  context 'when there is a cart that has not been interacted with for 3 hours' do
    let!(:no_interaction_cart) { create(:shopping_cart, user: user, last_interaction_at: 3.hours.ago) }
    let!(:active_cart) { create(:shopping_cart, user: user, last_interaction_at: 179.minutes.ago) }
  ...
  context 'when there is a cart that has not been interacted with for 7 days' do
    let!(:cart_to_destroy) { create(:shopping_cart, user: user, last_interaction_at: 7.days.ago) }
    let!(:abandoned_cart) { create(:shopping_cart, user: user, last_interaction_at: 167.hours.ago, abandoned: true) }
